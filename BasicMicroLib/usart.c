...
#ifndef DMA_BLUETOOTH_TX_CHAN_ID
#define DMA_BLUETOOTH_TX_CHAN_ID 0U
#endif
#ifndef DMA_BLUETOOTH_RX_CHAN_ID
#define DMA_BLUETOOTH_RX_CHAN_ID 1U
#endif
#define USART_DMA_TX_CHAN_ID DMA_BLUETOOTH_TX_CHAN_ID
#define USART_DMA_RX_CHAN_ID DMA_BLUETOOTH_RX_CHAN_ID
...
static void USART_StartRxDma(void)
{
	...
	DL_DMA_setSrcAddr(DMA, USART_DMA_RX_CHAN_ID,
					  (uint32_t)&Bluetooth_INST->RXDATA);
	...
	DL_UART_Main_clearInterruptStatus(Bluetooth_INST,
...
static void USART_DrainRxFifoToCallback(void)
{
	...
	while (!DL_UART_Main_isRXFIFOEmpty(Bluetooth_INST) &&
		   (drained < USART_RX_FIFO_DRAIN_LIMIT)) {
		USART_HandleRxByte(DL_UART_Main_receiveData(Bluetooth_INST));
		drained++;
	}
}
...
static void USART_StartTxDmaLocked(void)
{
	...
	DL_DMA_setDestAddr(DMA, USART_DMA_TX_CHAN_ID,
					   (uint32_t)&Bluetooth_INST->TXDATA);
...
void USART_Init(void)
{
	...
	// SysConfig 已完成蓝牙 UART1/DMA 通道配置，这里只设置运行期缓冲。
	DL_UART_Main_enableFIFOs(Bluetooth_INST);
	DL_UART_Main_setTXFIFOThreshold(Bluetooth_INST,
									DL_UART_MAIN_TX_FIFO_LEVEL_ONE_ENTRY);
	DL_UART_Main_setRXFIFOThreshold(Bluetooth_INST,
									DL_UART_MAIN_RX_FIFO_LEVEL_ONE_ENTRY);
	DL_UART_Main_setRXInterruptTimeout(Bluetooth_INST, 15);
	DL_UART_Main_disableInterrupt(Bluetooth_INST, DL_UART_MAIN_INTERRUPT_RX);
	DL_UART_Main_enableDMAReceiveEvent(Bluetooth_INST,
									   DL_UART_MAIN_DMA_INTERRUPT_RX);
	DL_UART_Main_enableDMATransmitEvent(Bluetooth_INST);
...
void USART_IRQHandler(void)
{
	...
	do {
		pending = DL_UART_Main_getPendingInterrupt(Bluetooth_INST);
		switch (pending) {
