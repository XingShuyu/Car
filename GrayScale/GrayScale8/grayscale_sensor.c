...
void Grayscale8_Sensor_Read_All(bool *sensor_values) {
	uint8_t i;
	for (i = 0; i < GRAYSCALE8_SENSOR_CHANNELS; i++) {
		sensor_values[i] = Grayscale8_Read_Channel_Stable(7 - i);
	}
}
...
void Grayscale8_Sensor_Read_Main(bool *sensor_values) {
	uint8_t i;
	for (i = 2; i < 6; i++) {
		sensor_values[i] = Grayscale8_Read_Channel_Stable(i);
	}
}
...
bool Grayscale8_Sensor_Read_Single(uint8_t channel) {
	if (channel >= GRAYSCALE8_SENSOR_CHANNELS) {
		return 0; // 无效通道 // Invalid channel
	}
	return Grayscale8_Read_Channel_Stable(channel);
}
